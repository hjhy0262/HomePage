	...
	<ul>
		<c:choose>
			...
			<c:when test="${step eq '2'}">
				<li>
					<h3>c:import Tag : jsp 'include'와 유사. 서버 내부 또는 서버 외부의 모든 자원
						컨텐츠를 포함하는 추가 기능이 있음</h3>
					<h4>기본문법 : &#60;c:import url="URL주소 값"
						charEncoding="utf-8"/&#62;</h4> 
			...
			<c:when test="${step eq '3'}">
				<li>
					...
					<h4>기본문법 : &#60;c:set var="변수형" value="값"/&#62;</h4>
			...
			<c:when test="${step eq '4'}">
				<li>
					<h3>c:if Tag : 조건문 중의 하나. JAVAJ에서 사용하는 If와 동일</h3>
			...
			<c:when test="${step eq '8'}">
				<li>
					...
					URL 2 : <c:url var="link2" value="/temp/jstlImport.do">
							...
							<br/>
					<a href="${link1}" target="_blank" title="새창열기">링크 1 번</a><br/>
			...
			<c:when test="${step eq '18'}">
				<li>
					<h3>fn:replace() : 모든 문자열을 다른 문자열 스퀀스로 바꿈</h3>
